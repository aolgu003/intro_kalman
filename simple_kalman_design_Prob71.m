function simple_kalman_design_Prob71
...
for t = time' 
    ...
    if mod(t, Tk) == 0 && t ~= 0
        [P, x, r] = measurement_update(P, H, x, R, utilde(i+1));
        residuals(j) = r;
        j = j + 1;
    end
...
subplot(4, 1, 2)
plot(data.data.t, x_vec(1,:), '.k', data.data.t, state_std(1,:),'b',data.data.t, -state_std(1,:),'b')
title('Position estimates vs Time')

subplot(4, 1, 3)
plot(data.data.t, x_vec(2,:), '.k', data.data.t, state_std(2,:),'b',data.data.t, -state_std(2,:),'b')
title('Velocity estimate')

subplot(4, 1, 4)
L = length(x_vec);
b = x_vec(3,L);
plot(data.data.t, x_vec(3,:), '.k', data.data.t, b+state_std(3,:),'b',data.data.t, b-state_std(3,:),'b')
title('Bias estimate')
